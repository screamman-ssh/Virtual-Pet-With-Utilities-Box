...
void display_calendar() {
  ...
  //เเสดงผล
  if(RTC.read(tm)){
    graphic.setBackground(calendar_mode? (uint32_t) 0x00000000 : day_color[tm.Wday - 1]); //เปลี่ยนสีตามวันในสัปดาห์
    graphic.drawWithColor(number3x5_data[tm.Day/10],calendar_mode? day_color[tm.Wday - 1] :0xffeaeaea ,5,3,2,1);
    graphic.drawWithColor(number3x5_data[tm.Day%10],calendar_mode? day_color[tm.Wday - 1] :0xffeaeaea ,5,3,6,1);
